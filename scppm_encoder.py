...
from encoder_functions import (bit_interleave, channel_interleave, convolve,
                               ...
                               zero_terminate, slot_map)

from ppm_parameters import (BIT_INTERLEAVE, CHANNEL_INTERLEAVE,
                            GREYSCALE, PAYLOAD_TYPE, M, slot_factor, m,
...
def SCPPM_encoder(information_blocks, save_encoded_sequence_to_file=True):
    ...
    # The encoded message can be saved to a file, to compare the BER before and after decoding
...
def postprocess_ppm_symbols(PPM_symbols, q=1):
    ...
    # Attach Codeword Synchronisation Markerk (CSM) to each codeword of 15120/m PPM symbols
    CSM = get_csm(M=M)
    
    ppm_mapped_message_with_csm = np.zeros(len(PPM_symbols) + len(CSM) * num_codewords, dtype=int)
    for i in range(num_codewords):
        prepended_codeword = np.hstack((CSM, PPM_symbols[i * symbols_per_codeword:(i + 1) * symbols_per_codeword]))
